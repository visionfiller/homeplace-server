...
class ReservationView(ViewSet):
    ...
    @action(methods=['get'], detail=False)       
    def my_sent_swaps(self, request):
        """find the swaps that the user signed has sent"""
        try:
            swaps = Reservation.objects.filter(swapper__user=request.auth.user)
            status_id = request.query_params.get('status', None)
    ...
    def ownercomplete(self, request, pk):
        ...
        try:
            swapper = Swapper.objects.get(user=request.auth.user)
            reservation= Reservation.objects.get(pk=pk, property__owner_id = swapper.id)
    ...
    def swappercomplete(self, request, pk):
        ...
        try:
            swapper = Swapper.objects.get(user=request.auth.user)
            reservation= Reservation.objects.get(pk=pk, swapper_id=swapper.id)
            reservation.completed = True
            reservation.save()
            return Response(None, status=status.HTTP_204_NO_CONTENT)
        except Reservation.DoesNotExist as ex:
